...
const std::string info = "IbeoSdkLaneEvaluation";
...
LaneEvaluation::~LaneEvaluation() {
	...
	for(std::vector<LaneFrameStatistic>::const_iterator it =m_frameStatistics.begin(); it != m_frameStatistics.end(); ++it){
		m_evalFile<<it->m_leftLanePosition<<";"<<it->m_leftLanePositionRef<<";"<<it->m_maxErrorLeft<<";"
				<<it->m_rightLanePosition<<";"<<it->m_rightLanePositionRef<<";"<<it->m_maxErrorRight<<std::endl;
	}
                             ...
                             const LogPolygonList2dFloat& lane)
{
	...
	for(LogPolygonList2dFloat::const_iterator it = lane.getLogList().begin(); it != lane.getLogList().end(); ++it){

		uint16_t foundMatch=0;
		uint16_t foundMissMatch=0;

		for(Polygon2dFloat::const_iterator ot=it->getPoly().begin(); ot != it->getPoly().end(); ++ot) {
			bool inLane=false;
...
void file_evaluation(const std::string& dutFilename, const std::string& refFileName)
{
	...
	if (dutFile.isOpen() && refFile.isOpen()) {
		...
		while (dutFile.isGood() && refFile.isGood()) {
			...
			while((db==NULL && refFile.isGood()) || (refFile.isGood() && db->getDataType()!=frameDataType));
			...
			while((db==NULL && dutFile.isGood()) || (dutFile.isGood() && db->getDataType()!=frameDataType));
