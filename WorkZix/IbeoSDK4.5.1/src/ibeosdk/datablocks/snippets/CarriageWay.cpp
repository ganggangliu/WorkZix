...
void CarriageWay::resolveConnections(const CarriageWayPtr& instance)
{
	...
	for (CarriageWaySegmentMap::const_iterator it = m_segments.begin(); it != m_segments.end(); it++)
	{
		...
		if (cws)
		{
			for(LaneMap::const_iterator itlane = cws->getLanes().begin(); itlane != cws->getLanes().end(); ++itlane)
			{
	...
	for (CarriageWaySegmentMap::const_iterator it = m_segments.begin(); it != m_segments.end(); ++it)
	{
		...
		if (cws)
		{
			for(LaneMap::const_iterator itlane = cws->getLanes().begin(); itlane != cws->getLanes().end(); ++itlane)
			{
				...
				if (lane)
				{
					for (LaneSegmentMap::const_iterator itLaneSeg = lane->getSegments().begin(); itLaneSeg != lane->getSegments().end(); ++itLaneSeg)
					{
