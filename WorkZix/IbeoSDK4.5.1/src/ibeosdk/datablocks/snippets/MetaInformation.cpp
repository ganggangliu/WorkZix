...
bool MetaInformation::deserialize(std::istream& is)
{
	...
	readBE(is, m_type);
	//std::cerr << "D: after type @:" << (is.tellg() - startPos) << "  Type: " << m_type << std::endl;
	readBE(is, m_timeStamp);
	//std::cerr << "D: after tms @:" << (is.tellg() - startPos) << std::endl;
	uint32_t payloadSize;
	readBE(is, payloadSize);
	//std::cerr << "D: after pls @:" << (is.tellg() - startPos) << "  PayloadSz: " << payloadSize << std::endl;
...
bool MetaInformation::serialize(std::ostream& os) const
{
	...
	writeBE(os, m_type);
	//std::cerr << "S: after type @:" << (os.tellp() - startPos) << "  Type: " << m_type << std::endl;
	writeBE(os, m_timeStamp);
	//std::cerr << "S: after tms @:" << (os.tellp() - startPos) << std::endl;
	writeBE(os, this->getSerializedPayloadSize());
	//std::cerr << "S: after pls @:" << (os.tellp() - startPos)  << "  PayloadSz: " << this->getSerializedPayloadSize() << std::endl;
