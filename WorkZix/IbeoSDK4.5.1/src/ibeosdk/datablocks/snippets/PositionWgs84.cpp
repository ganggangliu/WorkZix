...
void PositionWgs84::transformToTangentialPlane(const PositionWgs84& origin, double& pdX, double& pdY) const
{
	...
	this->transformToTangentialPlane(dOriginLat, dOriginLong, pdX, pdY);
}

//======================================================================

void PositionWgs84::transformToTangentialPlane(const double dOriginLat, const double dOriginLong,
                                               double& pdX, double& pdY) const
{
	PositionWgs84::transformToTangentialPlane(dOriginLat, dOriginLong,
	                                          this->getLatitudeInRad(), this->getLongitudeInRad(), pdX, pdY);
}

//======================================================================

void PositionWgs84::transformToTangentialPlane(const double originsLatitudeInRad, const double originsLongitudeInRad,
...
void PositionWgs84::transformToTangentialPlane(const EcefPoint& originRefPointsEcef,
                                               const double pointsLatitudeInRad, const double pointsLongitudeInRad,
                                               double& pointsX, double& pointsY)
{
	// some constants for the ellipsoid called earth
	const double a = 6378137.0;           // SemiMajorAxis
	const double b = 6356752.31424518;    // SemiMinorAxis
	//const double f = 1.0/298.257223563; // ellipsoid flattening
	const double ea2 = 1.0 - (b*b)/(a*a); // first eccentricity
	//const double eb2 = (a*a)/(b*b) - 1.0; // second eccentricity
...
void PositionWgs84::transformFromTangentialPlane(const double x, const double y, const PositionWgs84& origin)
{
	double lon, lat;
	PositionWgs84::transformFromTangentialPlane(origin.getLatitudeInRad(), origin.getLongitudeInRad(), x, y, lat, lon);
//	resetToDefault();
 ...
 *///-------------------------------------------------------------------
void PositionWgs84::transformFromTangentialPlane(const double originsLatitudeInRad, const double originsLongitudeInRad,
...
void PositionWgs84::transformToTangentialPlaneWithHeading(const PositionWgs84& origin, double& x, double& y) const
{
	double rotatedX;
	double rotatedY;
	transformToTangentialPlane(origin, rotatedX, rotatedY);
...
void PositionWgs84::transformFromTangentialPlaneWithHeading(const double x, const double y, const PositionWgs84& origin)
{
	...
	transformFromTangentialPlane(rotatedX, rotatedY, origin);
