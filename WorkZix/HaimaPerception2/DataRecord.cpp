...
CLidarDataOpr g_DataLog("");

string g_szChNameRadar = "LIDAR_RADAR_INFO";
CLcmRevicer<LIDAR_RADAR_INFO> g_LcmRadar(g_szChNameRadar);
string g_szChNameIbeoObjectsList = "IBEO_OBJECTS_LIST";
CLcmRevicer<IBEO_OBJECTS_LIST> g_LcmIbeoObjects(g_szChNameIbeoObjectsList);
string g_szChNameIbeoCloudPoints = "LIDAR_CLOUD_POINTS";
CLcmRevicer<LIDAR_CLOUD_POINTS> g_LcmIbeoCloudPoints(g_szChNameIbeoCloudPoints);
...
void DrawIbeoDataPoints(Mat& img, LIDAR_CLOUD_POINTS* pIbeoPoints, double dHead, Point2d T, Scalar color);
void DrawIbeoDataObjects(Mat& img, IBEO_OBJECTS_LIST* pIbeoObjects, double dHead, Point2d T, Scalar color = Scalar::all(-1));
...
IBEO_OBJECTS_LIST g_IbeoObjects;
LIDAR_CLOUD_POINTS g_IbeoPoints;
...
{
	IBEO_OBJECTS_LIST* pIbeoObjList = (IBEO_OBJECTS_LIST*) pData;

	g_IbeoObjects = *pIbeoObjList;
	CGpsManager::GPS_IMU_INFO imu;
	g_IMU.GetData(imu);
	GPS_DATA imu_;
	imu_.GPS_ALTITUDE = imu.Altitude;
	imu_.GPS_HEADING = imu.Heading;
	imu_.GPS_LATITUDE = imu.Lattitude;
	imu_.GPS_LONGITUDE = imu.Longitude;
	imu_.GPS_PITCH = imu.Pitch;
	imu_.GPS_ROLL = imu.Roll;
	imu_.GPS_TIME = imu.GPSTime;
	imu_.GPS_VE = imu.Ve;
	imu_.GPS_VN = imu.Vn;
	imu_.GPS_VU = imu.Vu;
	imu_.GPS_HDOP = imu.Status;
	imu_.GPS_NSV1 = imu.NSV1;
	imu_.GPS_NSV2 = imu.NSV2;
	long nInd = g_DataLog.WriteLog(pIbeoObjList,&imu_);
	printf("Log object data: %d\n",nInd);
...
{
	LIDAR_CLOUD_POINTS* pIbeoCloudPoints = (LIDAR_CLOUD_POINTS*) pData;
	...
	imu_.GPS_NSV1 = imu.NSV1;
	imu_.GPS_NSV2 = imu.NSV2;
	long nInd = g_DataLog.WriteLog(pIbeoCloudPoints,&imu_);
	printf("Log Points data: %d\n",nInd);
...
void DrawIbeoDataPoints(Mat& img, LIDAR_CLOUD_POINTS* pIbeoPoints, double dHead, Point2d T, Scalar color)
{
	...
	for (int i = 0; i < pIbeoPoints->points.size(); i++)
	{
		if (pIbeoPoints->points[i].layer <= -1)
		{
			continue;
		}
		double X = sin((-1.f*pIbeoPoints->points[i].angle/32.f+dHead)/180.f*CV_PI)*pIbeoPoints->points[i].distance + T.x * 1000.0;
		double Z = cos((-1.f*pIbeoPoints->points[i].angle/32.f+dHead)/180.f*CV_PI)*pIbeoPoints->points[i].distance + T.y * 1000.0;
...
void DrawIbeoDataObjects(Mat& img, IBEO_OBJECTS_LIST* pIbeoObjects, double dHead, Point2d T, Scalar color)
{
	if (img.data == NULL)
	{
		img = Mat::zeros((g_nGridFront+g_nGridBack)/g_nGridSize+1,g_nGridSide*2/g_nGridSize+1,CV_8UC3);
	...
	{
		IBEO_OBJECT& ObjRef = pIbeoObjects->Objects[i];
		//Create color
		bool isRandMatchColor = color == Scalar::all(-1);
		Scalar color_ = isRandMatchColor ? g_ColorList[ObjRef.Id] : color;
		//Draw counter points
		vector< IBEO_POINT_CART >& ContPointsRef = ObjRef.ContourPoints;
		...
		//Draw objects rect
		double X_ = ObjRef.ObjectBoxCenter.x;
		double Y_ = ObjRef.ObjectBoxCenter.y;
		double X = X_*cos(dHead/180.f*CV_PI) - Y_*sin(dHead/180.f*CV_PI) + T.x*1000.0;
		double Y = X_*sin(dHead/180.f*CV_PI) + Y_*cos(dHead/180.f*CV_PI) + T.y*1000.0;
		Point2f center(X, Y);
		Point2f center_((center.x + g_nGridSide) / g_nGridSize, (g_nGridFront - center.y) / g_nGridSize);
		Point2f size(ObjRef.ObjectBoxSize.x, ObjRef.ObjectBoxSize.y);
		Point2f size_(size.x / g_nGridSize, size.y / g_nGridSize);
		RotatedRect rectOrient(center_, size_, -1.f*ObjRef.ObjectBoxOrientaion-dHead);
		Point2f vertices_[4];
