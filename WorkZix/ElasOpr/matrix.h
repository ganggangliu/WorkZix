...
typedef double FLOAT_M;      // double precision
//typedef float  FLOAT_M;    // single precision
...
public:
  ...
  Matrix (const int32_t m,const int32_t n,const FLOAT_M* val_); // init mxn matrix with values from array 'val'
  ...
  void getData(FLOAT_M* val_,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);
  ...
  void setVal(FLOAT_M s,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);

  // set (part of) diagonal to scalar, -1 as end replaces whole diagonal
  void setDiag(FLOAT_M s,int32_t i1=0,int32_t i2=-1);
  ...
  static Matrix rotMatX(const FLOAT_M &angle);
  static Matrix rotMatY(const FLOAT_M &angle);
  static Matrix rotMatZ(const FLOAT_M &angle);
  ...
  Matrix  operator* (const FLOAT_M &s);  // multiply with scalar
  Matrix  operator/ (const Matrix &M); // divide elementwise by matrix (or vector)
  Matrix  operator/ (const FLOAT_M &s);  // divide by scalar
  Matrix  operator- ();                // negative matrix
  Matrix  operator~ ();                // transpose
  FLOAT_M   l2norm ();                   // euclidean norm (vectors) / frobenius norm (matrices)
  FLOAT_M   mean ();                     // mean of all elements in matrix
  ...
  FLOAT_M  det ();                                             // returns determinant of matrix
  bool   solve (const Matrix &M,FLOAT_M eps=1e-20);            // solve linear system M*x=B, replaces *this and M
  bool   lu(int32_t *idx, FLOAT_M &d, FLOAT_M eps=1e-20);        // replace *this by lower upper decomposition
...
private:
  ...
  void releaseMemory ();
  inline FLOAT_M pythag(FLOAT_M a,FLOAT_M b);
