...
static FLOAT_M sqrarg;
#define SQR(a) ((sqrarg=(a)) == 0.0 ? 0.0 : sqrarg*sqrarg)
static FLOAT_M maxarg1,maxarg2;
...
Matrix::Matrix (const int32_t m_,const int32_t n_,const FLOAT_M* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
Matrix::Matrix (const Matrix &M) {
  ...
  for (int32_t i=0; i<M.m; i++)
    memcpy(val[i],M.val[i],M.n*sizeof(FLOAT_M));
}
...
void Matrix::getData(FLOAT_M* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
void Matrix::setVal(FLOAT_M s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(FLOAT_M s,int32_t i1,int32_t i2) {
...
Matrix Matrix::rotMatX (const FLOAT_M &angle) {
  FLOAT_M s = sin(angle);
  FLOAT_M c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +1;
...
Matrix Matrix::rotMatY (const FLOAT_M &angle) {
  FLOAT_M s = sin(angle);
  FLOAT_M c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::rotMatZ (const FLOAT_M &angle) {
  FLOAT_M s = sin(angle);
  FLOAT_M c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::operator* (const FLOAT_M &s) {
  Matrix C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
bool Matrix::solve (const Matrix &M, FLOAT_M eps) {
...
bool Matrix::lu(int32_t *idx, FLOAT_M &d, FLOAT_M eps) {
  ...
  int32_t i,imax,j,k;
  FLOAT_M   big,dum,sum,temp;
  FLOAT_M* vv = (FLOAT_M*)malloc(n*sizeof(FLOAT_M)); // vv stores the implicit scaling of each row.
  d = 1.0;
...
void Matrix::svd(Matrix &U2,Matrix &W,Matrix &V) {
  ...
  FLOAT_M* w   = (FLOAT_M*)malloc(n*sizeof(FLOAT_M));
  FLOAT_M* rv1 = (FLOAT_M*)malloc(n*sizeof(FLOAT_M));

  int32_t flag,i,its,j,jj,k,l,nm;
  FLOAT_M   anorm,c,f,g,h,s,scale,x,y,z;
  ...
  for (k=n-1;k>=0;k--) { // Diagonalization of the bidiagonal form: Loop over singular values,
    for (its=0;its<30;its++) { // and over allowed iterations.
      ...
      for (l=k;l>=0;l--) { // Test for splitting.
        nm = l-1;
        if ((FLOAT_M)(fabs(rv1[l])+anorm) == anorm) { flag = 0; break; }
        if ((FLOAT_M)(fabs( w[nm])+anorm) == anorm) { break; }
      }
      if (flag) {
        ...
        for (i=l;i<=k;i++) {
          f = s*rv1[i];
          rv1[i] = c*rv1[i];
          if ((FLOAT_M)(fabs(f)+anorm) == anorm) break;
          g = w[i];
          h = pythag(f,g);
          w[i] = h;
          h = 1.0/h;
  ...
  int32_t s2,inc=1;
  FLOAT_M   sw;
  FLOAT_M* su = (FLOAT_M*)malloc(m*sizeof(FLOAT_M));
  FLOAT_M* sv = (FLOAT_M*)malloc(n*sizeof(FLOAT_M));
...
void Matrix::allocateMemory (const int32_t m_,const int32_t n_) {
  ...
  val    = (FLOAT_M**)malloc(m*sizeof(FLOAT_M*));
  val[0] = (FLOAT_M*)calloc(m*n,sizeof(FLOAT_M));
...
FLOAT_M Matrix::pythag(FLOAT_M a,FLOAT_M b) {
  FLOAT_M absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)
