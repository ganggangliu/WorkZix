...
{
    m_GpsData = LoadGpsData(m_Param.szGpsPath);
...
cv::Mat CProjectOpr::LoadGpsData(char* pszFile)
{
    vector<vector<double> > Temp;
    ifstream fs(pszFile,ios::in);
    ...
    cv::Mat GpsOut = cv::Mat::zeros(Temp.size(),Temp[0].size(),CV_64FC1);
...
long CProjectOpr::GetStereoImgByGpsFile(cv::Mat* pSteMat, cv::Mat& GpsData, int Color)
{
	if (m_nReadInd < m_VecImgPathL.size() && m_nReadInd < m_VecImgPathR.size())
	{
        cv::Mat L = cv::imread(m_VecImgPathL[m_nReadInd],Color);
        cv::Mat R = cv::imread(m_VecImgPathR[m_nReadInd],Color);
...
long CProjectOpr::GetStereoImgByDoc(cv::Mat* pSteMat, int Color)
{
	for (int i = m_nReadInd; i < 1000000; i++)
	{
		char szPathL[MAX_PATH] = {0};
		char szPathR[MAX_PATH] = {0};
		sprintf(szPathL,"%s%s%d%s",m_Param.szProjPath,m_Param.szImgNameHeadL,i,m_Param.szImgNameTail);//
		sprintf(szPathR,"%s%s%d%s",m_Param.szProjPath,m_Param.szImgNameHeadR,i,m_Param.szImgNameTail);
        cv::Mat L = cv::imread(szPathL,Color);
        cv::Mat R = cv::imread(szPathR,Color);
		if (L.data != NULL && R.data != NULL)
		{
			L.copyTo(pSteMat[0]);
			R.copyTo(pSteMat[1]);
		...
		sprintf(szPathL,"%s%s%06d%s",m_Param.szProjPath,m_Param.szImgNameHeadL,i,m_Param.szImgNameTail);//
		sprintf(szPathR,"%s%s%06d%s",m_Param.szProjPath,m_Param.szImgNameHeadR,i,m_Param.szImgNameTail);
        L = cv::imread(szPathL,Color);
        R = cv::imread(szPathR,Color);
		if (L.data != NULL && R.data != NULL)
		{
			L.copyTo(pSteMat[0]);
			R.copyTo(pSteMat[1]);
...
long CProjectWriter::Init(CProjectParam Param)
{
    ...
    strcpy(szFilePath,m_Param.szModuleFileName);
//	GetModuleFileNameA(NULL,szFilePath,MAX_PATH);
	...
	*p = 0x00;
	...
	AddTimeTail(szTime);
	sprintf(m_Param.szProjPath,"%s%s\\",szFilePath,szTime);//
	sprintf(m_szAutoSavePath,"%sAutoSave\\",m_Param.szProjPath);//
	sprintf(m_szManualSavePath,"%sManualSave\\",m_Param.szProjPath);//
	sprintf(m_szGpsLogAuto,"%sAutoSave\\GpsLogAuto.log",m_Param.szProjPath);//
	sprintf(m_szGpsLogManual,"%sManualSave\\GpsLogManual.log",m_Param.szProjPath);//
...
//	CreateDirectoryA(m_szManualSavePath,NULL);
    mkdir(m_Param.szProjPath, 777);
    mkdir(m_szAutoSavePath, 777);
    mkdir(m_szManualSavePath, 777);
...
long CProjectWriter::SaveImageAuto(long nRecInd, cv::Mat* pImg, LCM_GPS_DATA GpsData)
{
	if (m_bIsCreated)
	{
	}
    ...
    sprintf(szPathName,"%s%s%06ld%s",m_szAutoSavePath,m_Param.szImgNameHeadL,m_nSaveIndAuto,m_Param.szImgNameTail);
    cv::imwrite(szPathName,pImg[0]);
	printf("%s\n",szPathName);
    sprintf(szPathName,"%s%s%06ld%s",m_szAutoSavePath,m_Param.szImgNameHeadR,m_nSaveIndAuto,m_Param.szImgNameTail);
    cv::imwrite(szPathName,pImg[1]);
	printf("%s\n",szPathName);
    sprintf(szPathName,"%06ld,%d,%.3f,%.3f,%.3f,%.3f,%.7f,%.7f,%.3f,%.3f,%.3f,%.3f,%.3f,%d,%d,%.2x,%d\n",
		m_nSaveIndAuto,
		nRecInd,
		GpsData.GPS_TIME,
		GpsData.GPS_HEADING,
...
long CProjectWriter::SaveImageAutoVideo(long nRecInd, cv::Mat* pImg, LCM_GPS_DATA GpsData)
{
    ...
    IplImage IplImg0(pImg[0]);
    IplImage IplImg1(pImg[1]);
    cvWriteFrame(m_writerL,&IplImg0);
    cvWriteFrame(m_writerR,&IplImg1);
...
long CProjectWriter::SaveImageManual(long nRecInd, cv::Mat* pImg, LCM_GPS_DATA GpsData)
{
	if (m_bIsCreated)
	{
	}
	...
	char szPathName[MAX_PATH] = {0};
    sprintf(szPathName,"%s%s%06ld%s",m_szManualSavePath,m_Param.szImgNameHeadL,m_nSaveIndManual,m_Param.szImgNameTail);
    cv::imwrite(szPathName,pImg[0]);
	printf("%s\n",szPathName);
    sprintf(szPathName,"%s%s%06ld%s",m_szManualSavePath,m_Param.szImgNameHeadR,m_nSaveIndManual,m_Param.szImgNameTail);
    cv::imwrite(szPathName,pImg[1]);
	printf("%s\n",szPathName);
...
CLidarDataOpr::CLidarDataOpr(char* pszModulePath)
{
    char szszModulePath[MAX_PATH] = {0};
    strcpy(szszModulePath, pszModulePath);
    char* p = strrchr(szszModulePath,'/');
    *p = 0x00;
    strcpy(m_szPath, szszModulePath);
    strcat(m_szPath, "/");
    strcpy(m_szPathRead, pszModulePath);
    m_nInd = 0;
	m_fWrite = NULL;
	m_fRead = NULL;
...
long CLidarDataOpr::WriteLog(LCM_IBEO_CLOUD_POINTS* pLidarPoints, LCM_GPS_DATA* pImuInfo)
{
    ...
    sprintf(szPathCloudPt,"%s%06d.lidar",m_szPath,m_nInd);
    sprintf(szPathImu,"%s%06d.imu",m_szPath,m_nInd);
...
long CLidarDataOpr::ReadLog(LCM_IBEO_CLOUD_POINTS* pLidarPoints, LCM_GPS_DATA* pImuInfo)
{
	char szPathCloudPt[MAX_PATH] = {0};
	char szPathImu[MAX_PATH] = {0};
    sprintf(szPathCloudPt,"%s%06d.lidar",m_szPathRead,m_nInd);
    sprintf(szPathImu,"%s%06d.imu",m_szPathRead,m_nInd);
...
long CLidarDataOpr::WriteLog(LCM_SENSOR_FUSION_PACKAGE* pPackData)
{
    ...
    sprintf(szPath,"%s%06d.log",m_szPath,m_nInd);
...
{
	char szPath[MAX_PATH] = {0};
    sprintf(szPath,"%s%06d.log",m_szPathRead,m_nInd);
