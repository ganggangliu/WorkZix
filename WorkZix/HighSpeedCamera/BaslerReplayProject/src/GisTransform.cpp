...
Point2d Point2GisWgsPointRotate(Point2d& pt0, Point2d& pt1, double dHeading)
{
	Point2d pt1_ = GisCartPoint2PointRotate(Point2d(0.0,0.0), pt1, dHeading);
	Point2d PtOut;
	PtOut.x = pt1_.y/111319.55 + pt0.x;
	PtOut.y = pt1_.x/111319.55/cos(pt0.x/180.0*CV_PI) + pt0.y;
	return PtOut;
}
...
{
	Point2d ptDelta;
	double dCos = cos(dHeadingVehicle/180.0*CV_PI);
	double dSin = sin(dHeadingVehicle/180.0*CV_PI);
	ptDelta.x = dSin*Target.x + dCos*Target.y;
	ptDelta.y = -dCos*Target.x + dSin*Target.y;

	return ptVehicle+ptDelta;
}