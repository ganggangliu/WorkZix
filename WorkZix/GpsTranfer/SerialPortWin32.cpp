...
CSerialPort::CSerialPort()
{
    ...
    m_hEventUsedForComEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
    m_hEventUsedForWrite = CreateEvent(NULL, TRUE, FALSE, NULL);
    m_hEventUsedForRead = CreateEvent(NULL, TRUE, FALSE, NULL);
...
{
    ClosePort();
    CloseHandle(m_hEventUsedForComEvent);
    CloseHandle(m_hEventUsedForWrite);
    CloseHandle(m_hEventUsedForRead);
...
//		COM_ERR("[ERR] SetCommState err\n");
    ...
    bIsSuccess = SetSerialComMask(EV_RXCHAR);
    if (FALSE == bIsSuccess) {
        LeaveCriticalSection(&m_csCommunicationSync);
//        COM_ERR("[ERR] SetSerialComMask err\n");
        ClosePort();
        return (FALSE);
    }
...
void CSerialPort::Start(void)
{
    EnterCriticalSection(&m_csCommunicationSync);
    m_bRunFlag = true;
//    COM_DEBUG(5, ("COM: CSerialPort::Start, run flag(%d)\n", m_bRunFlag));
    LeaveCriticalSection(&m_csCommunicationSync);
}

void CSerialPort::Stop(void)
{
    EnterCriticalSection(&m_csCommunicationSync);
    m_bRunFlag = false;
//    COM_DEBUG(5, ("COM: CSerialPort::Stop, run flag(%d)\n", m_bRunFlag));
    SetEvent(m_hEventUsedForComEvent);
    SetEvent(m_hEventUsedForWrite);
    SetEvent(m_hEventUsedForRead);
    LeaveCriticalSection(&m_csCommunicationSync);
}
...
BOOL CSerialPort::WaitSerialComEvent(DWORD& evtMask_)
{
    ...
    ResetEvent(m_hEventUsedForComEvent);
    os.hEvent = m_hEventUsedForComEvent;
    LeaveCriticalSection(&m_csCommunicationSync);
	...
	if(!(bRet = WaitCommEvent(m_hComm, &evtMask_, &os))) {
		if (GetLastError() == ERROR_IO_PENDING){
            ...
            if (FALSE == bRet) {
//                COM_ERR("[ERR]COM: CSerialPort::WaitSerialComEvent, GetOverlappedResult err, run flag(%d)\n", m_bRunFlag);
            }
		}
	}
...
DWORD CSerialPort::ReadData(char* pData, int length, DWORD msec_)
{
	COMSTAT ComStat;
	DWORD dwErrorFlags;
	OVERLAPPED osRead;
    ...
    ResetEvent(m_hEventUsedForRead);
    osRead.hEvent = m_hEventUsedForRead;
    LeaveCriticalSection(&m_csCommunicationSync);
...
//                    COM_ERR("[ERR]COM: CSerialPort::ReadData, read serial port timeout\n");
				} else {
					bReadStatus = GetOverlappedResult(m_hComm, &osRead, &dwBytesRead, TRUE);
					if (FALSE == bReadStatus) {
//                        COM_ERR("[ERR]COM: CSerialPort::ReadData, GetOverlappedResult err, read byte(%d), run flag(%d)\n",
//                                dwBytesRead, m_bRunFlag);
					}
				}
			}
		}
...
DWORD CSerialPort::WriteData(const char* pData, int length, DWORD msec_)
{
	COMSTAT ComStat;
	DWORD dwErrorFlags;
    ...
    ResetEvent(m_hEventUsedForWrite);
    osWrite.hEvent = m_hEventUsedForWrite;
    LeaveCriticalSection(&m_csCommunicationSync);
	//printf("hEvent=%p\n", osWrite.hEvent);
	...
	if(!bWriteStat) {
		...
		if (GetLastError() == ERROR_IO_PENDING) {
			...
			if (msec_ > 0) {
				...
				if (WAIT_OBJECT_0 != dwRet) {
//                    COM_ERR("[ERR]COM: CSerialPort::WriteData, write serial port timeout\n");
				} else {
					//printf("WaitForSingleObject==WAIT_OBJECT_0\n");
					bWriteStat = GetOverlappedResult(m_hComm, &osWrite, &dwBytesWritten, TRUE);
					//printf("length=%d dwBytesWritten=%d\n", length, dwBytesWritten);
					if (FALSE == bWriteStat) {
//                        COM_ERR("[ERR]COM: CSerialPort::WriteData, GetOverlappedResult err, write bytes(%d), run flag(%d)\n",
//                                dwBytesWritten, m_bRunFlag);
					}
				}
			}
		}
