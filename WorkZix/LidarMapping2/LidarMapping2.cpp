...
void WINAPI PackDataCallBack(void* pData)
{
	...
	printf("Long:%.7f, Lat:%.7f, Head:%.2f, VE:%.2f, VN:%.2f\n",
...
int main(int argc, char* argv[])
{
	...
	LidarMappingOpr.SetScale(80000,30000,30000,150);
//	LidarMappingOpr.SetCallBack(PackDataCallBack);
	...
	while(1)
	{
		CLidarMappingOpr::DATA_PACKAGE DataPack;
		long nRt = LidarMappingOpr.GetData(&DataPack);
		if (nRt < 0)
		{
			Sleep(1);
			continue;
		...
		printf("%d\n",DataPack.ObjectList.objects.size());

		PackDataCallBack(&DataPack);
