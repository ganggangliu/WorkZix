...
void __stdcall OnBinData(void* pData, void* pUser)
{
	...
	for (int i = 0; i < 24; i++)
	{
		long nCont = pVPL16Opr->m_CurDataRound.VecBlock.size();
		VPL16DataBlock& CurBlock = pVPL16Opr->m_CurData.DataBlock[i];
		if (nCont <= 0)
		{
			pVPL16Opr->m_CurDataRound.VecBlock.push_back(CurBlock);
			continue;
		}

		bool bIsLoop = false;
		VPL16DataBlock& LastBlock = pVPL16Opr->m_CurDataRound.VecBlock[nCont-1];
		if ((CurBlock.fAzimuth < LastBlock.fAzimuth) && 
			(pVPL16Opr->m_dStartAngle == 0.0))
		{
			bIsLoop = true;
		}
		else if ((CurBlock.fAzimuth > pVPL16Opr->m_dStartAngle) && 
			(LastBlock.fAzimuth <= pVPL16Opr->m_dStartAngle))
		{
			bIsLoop = true;
		}
		else
		...
		if (bIsLoop)
		{
			pVPL16Opr->OnRoundData(pVPL16Opr->m_CurDataRound);
			printf("%d\n",pVPL16Opr->m_CurDataRound.VecBlock.size());
			pVPL16Opr->m_CurDataRound.VecBlock.clear();
		}

		pVPL16Opr->m_CurDataRound.VecBlock.push_back(CurBlock);
	}

	//double dStart = pVPL16Opr->m_CurData.DataBlock[0].fAzimuth;
	//double dEnd = pVPL16Opr->m_CurData.DataBlock[23].fAzimuth;
	//printf("Start:%.5f, End:%.5f\n",dStart,dEnd);
	//if (dEnd < dStart)
	//{
	//	dStart -= 360.0;
	//	printf("Loop!!!\n");
	//	for (int i = 0; i < 24; i++)
	//	{
	//		printf("%d:%.3f\n",i,pVPL16Opr->m_CurData.DataBlock[i].fAzimuth);
	//	}
	//}
	//if (dStart <= pVPL16Opr->m_dStartAngle && dEnd > pVPL16Opr->m_dStartAngle)
	//{
	//	pVPL16Opr->OnRoundData(pVPL16Opr->m_CurDataRound);
	//	pVPL16Opr->m_CurDataRound.DataRound.clear();
	//}
	//pVPL16Opr->m_CurDataRound.DataRound.push_back(pVPL16Opr->m_CurData);


	//if (pVPL16Opr->m_DataCallBack)
	//{
	//	(*(pVPL16Opr->m_DataCallBack))(&(pVPL16Opr->m_CurData),pVPL16Opr->m_pUser);
	//}
}
...
{
	m_PointsCloud.Clear();
//	if (DataRound.DataRound.size()<75)
	{
		printf("DataRound miss part of data!!! : %d\n",DataRound.VecBlock.size());
	}
	for (int i = 0; i < DataRound.VecBlock.size(); i++)
	{
		...
		for (int j = 0; j < 16; j++)
		{
			...
			m_PointsCloud.CloudPoints.push_back(pt);
			m_PointsCloud.Reflect.push_back(Reflect);
		}
	}

	Mat Cpt(m_PointsCloud.CloudPoints.size(),3,CV_64F,m_PointsCloud.CloudPoints.data());
	m_OpenGL->ClearPoints();
	m_OpenGL->AddPointsRelative(Cpt.clone());
	...
	while(1)
	{
		...
		for (int i = 0; i < VecPack.size(); i++)
		{
			...
			for (int j = 0; j < 24; j++)
			{
				...
				VPL16DataBlock& CurBlock = PackT.DataBlock[i];
				if (nCont <= 0)
				{
				...
				else if ((CurBlock.fAzimuth > m_dStartAngle) && 
				...
				if (bIsLoop)
				{
					m_RoundBuf.AddData(CurDataRound);
