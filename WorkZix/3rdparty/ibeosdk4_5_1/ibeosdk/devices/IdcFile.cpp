...
bool IdcFile::writeHeader(const IbeoDataHeader& dh)
{
	...
	if (m_frameIndex.getFramingPolicy().isTrigger(dh.getDataType(), dh.getDeviceId())) {
		...
		ibeosdk::FrameIndexEntry entry(UINT64(m_strm.tellp()), dh.getTimestamp(), dh.getDeviceId());
...
{
	for(DataListenerBase::DataTypes::const_iterator it = listener->getAssociatedDataTypes().begin(); it != listener->getAssociatedDataTypes().end(); ++it){
