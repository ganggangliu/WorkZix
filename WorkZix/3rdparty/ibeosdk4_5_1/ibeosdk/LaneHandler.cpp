...
{
	PositionWgs84 p;
	p.transformFromTangentialPlane(point.getXPosition(), point.getYPosition(), point.getReferencePoint());
...
LanePosition LaneHandler::getLanePosition(const PositionWgs84& point) const
{
	...
	{
		for(CarriageWaySegmentMap::const_iterator segIt = (*it)->getSegments().begin();
				segIt != (*it)->getSegments().end(); ++segIt)
		{
	...
	for (std::vector<CarriageWaySegmentPtr>::iterator it = validCWSs.begin(); it != validCWSs.end(); it++)
	{
		for(LaneMap::const_iterator laneIt = (*it)->getLanes().begin(); laneIt != (*it)->getLanes().end(); ++laneIt)
		{
	...
	for (std::vector<LanePtr>::iterator it = validLanes.begin(); it != validLanes.end(); it++)
	{
		for(LaneSegmentMap::const_iterator segIt = (*it)->getSegments().begin(); segIt != (*it)->getSegments().end(); ++segIt)
		{
			BoundingRectangle mBox = segIt->second->getBoundingBox();
...
{
	double xs, ys;
	point.transformToTangentialPlaneWithHeading(segment->getStartPoint().getPoint(), xs, ys);
...
LanePosition LaneHandler::getReferenceLanePosition(const LanePosition& reference, const TransformationMatrix2dFloat& relativePosition, bool& insideReferenceLane, bool& success) const
{
	...
	objectPosition.transformFromTangentialPlaneWithHeading(relativePosition.getPosition().getX(), relativePosition.getPosition().getY(), reference.gpsPosition);
	...
	if (reference.laneSegment)
	{
		...
		for (CarriageWayList::const_iterator it_way = m_ways.begin(); it_way != m_ways.end(); ++it_way)
		{
			for(CarriageWaySegmentMap::const_iterator it_ws = (*it_way)->getSegments().begin(); it_ws != (*it_way)->getSegments().end(); ++it_ws)
			{
				for (LaneMap::const_iterator it_l = it_ws->second->getLanes().begin(); it_l != it_ws->second->getLanes().end(); ++it_l)
				{
					for(LaneSegmentMap::const_iterator it_ls = it_l->second->getSegments().begin(); it_ls != it_l->second->getSegments().end(); ++it_ls)
					{
		...
		if (!validLaneSegments.empty())
		{
			...
			float minOutsideDist = .0f;
			bool bi = false, bo = false;
			for (std::vector<LaneSegmentPtr>::const_iterator it = validLaneSegments.begin(); it != validLaneSegments.end(); ++it)
			{
				LaneSegmentPtr curSeg = *it;
				bool inside = false;
...
TransformationMatrix2dFloat LaneHandler::getRelativePosition(const LanePosition& reference, const float& longitudinalDistance, const float& lateralDistance, bool& success) const
{
	...
	if (startSeg)
	{
		...
		PositionWgs84 objectPosition;
		objectPosition.transformFromTangentialPlaneWithHeading(ldistance, lateralDistance, startSeg->getStartPoint().getPoint());

		// create matrix relative to reference
		double x, y;
		objectPosition.transformToTangentialPlaneWithHeading(reference.gpsPosition, x, y);
