...
long VisualOdometryStereo::GetMoveObj(std::vector<MoveObject>& VecMoveObj)
{
	vector<Matcher::p_match> matches = getMatches();
	vector<int32_t> valid = getInlierIndices();
	...
	std::vector<double> Vecxp(VecMatchBad.size());
	std::vector<double> Vecyp(VecMatchBad.size());
	std::vector<double> Veczp(VecMatchBad.size());
	for (int i = 0; i < VecMatchBad.size(); i++)
	{
//		W = ptT.dDisP/(-1.f*m_TT);
//		ptT.ptP3D.x = (ptT.ptLP.x - m_cx)/W;
//		ptT.ptP3D.y = (ptT.ptLP.y - m_cy)/W;
//		ptT.ptP3D.z = m_f/W;
		double W = (VecMatchBad[i].u1p - VecMatchBad[i].u2p)/param.base;
		Vecxp[i] = (VecMatchBad[i].u1p - param.calib.cu)/W;
		Vecyp[i] = (VecMatchBad[i].v1p - param.calib.cv)/W;
		Veczp[i] = param.calib.f/W;
	}
	
	Matrix poseN = getMotion();
	std::vector<double> Vecxp_c(VecMatchBad.size());
	std::vector<double> Vecyp_c(VecMatchBad.size());
	std::vector<double> Veczp_c(VecMatchBad.size());
	for (int i = 0; i < VecMatchBad.size(); i++)
	{
		FLOAT_M pt0s[4] = {Vecxp[i],Vecyp[i],Veczp[i],1};
		Matrix pt0(4,1,pt0s);
		Matrix pt1 = poseN*pt0;
		Vecxp_c[i] = pt1.val[0][0];
		Vecyp_c[i] = pt1.val[1][0];
		Veczp_c[i] = pt1.val[2][0];
	}

	std::vector<double> Vecxc(VecMatchBad.size());
	std::vector<double> Vecyc(VecMatchBad.size());
	std::vector<double> Veczc(VecMatchBad.size());
	for (int i = 0; i < VecMatchBad.size(); i++)
	{
		double W = (VecMatchBad[i].u1c - VecMatchBad[i].u2c)/param.base;
		Vecxc[i] = (VecMatchBad[i].u1c - param.calib.cu)/W;
		Vecyc[i] = (VecMatchBad[i].v1c - param.calib.cv)/W;
		Veczc[i] = param.calib.f/W;
	}

	std::vector<double> VecDx(VecMatchBad.size());
	std::vector<double> VecDy(VecMatchBad.size());
	std::vector<double> VecDz(VecMatchBad.size());
	for (int i = 0; i < VecMatchBad.size(); i++)
	{
		VecDx[i] = Vecxc[i] - Vecxp_c[i];
		VecDy[i] = Vecyc[i] - Vecyp_c[i];
		VecDz[i] = Veczc[i] - Veczp_c[i];
	}
