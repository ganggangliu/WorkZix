...
void Reconstruction::setCalibration (FLOAT_M f,FLOAT_M cu,FLOAT_M cv) {
  FLOAT_M K_data[9]       = {f,0,cu,0,f,cv,0,0,1};
  K                     = Matrix(3,3,K_data);
  FLOAT_M cam_pitch       = -0.08;
  FLOAT_M cam_height      = 1.6;
...
bool Reconstruction::refinePoint(const track &t,point3d &p) {
  ...
  J         = new FLOAT_M[6*num_frames];
  p_observe = new FLOAT_M[2*num_frames];
  p_predict = new FLOAT_M[2*num_frames];
...
double Reconstruction::rayAngle(const track &t,point3d &p) {
  ...
  Matrix v1 = c1-pt;
  Matrix v2 = c2-pt;
  FLOAT_M  n1 = v1.l2norm();
  FLOAT_M  n2 = v2.l2norm();
...
Reconstruction::result Reconstruction::updatePoint(const track &t,point3d &p,const FLOAT_M &step_size,const FLOAT_M &eps) {
...
bool Reconstruction::computePredictionsAndJacobian(const vector<Matrix>::iterator &P_begin,const vector<Matrix>::iterator &P_end,point3d &p) {
  ...
  for (vector<Matrix>::iterator P=P_begin; P<=P_end; P++) {
    ...
    FLOAT_M a  = P->val[0][0]*p.x+P->val[0][1]*p.y+P->val[0][2]*p.z+P->val[0][3];
    FLOAT_M b  = P->val[1][0]*p.x+P->val[1][1]*p.y+P->val[1][2]*p.z+P->val[1][3];
    FLOAT_M c  = P->val[2][0]*p.x+P->val[2][1]*p.y+P->val[2][2]*p.z+P->val[2][3];
    FLOAT_M cc = c*c;
...
void Reconstruction::testJacobian() {
  ...
  cout << P[1] << endll;
  J         = new FLOAT_M[6*2];
  p_observe = new FLOAT_M[2*2];
  p_predict = new FLOAT_M[2*2];
  
  point3d p_ref(0.1,0.2,0.3);
  
  FLOAT_M p_predict1[4];
  FLOAT_M p_predict2[4];
  point3d p1 = p_ref;
  ...
  for (int32_t i=0; i<3; i++) {
    ...
    memcpy(p_predict1,p_predict,4*sizeof(FLOAT_M));
    computePredictionsAndJacobian(P.begin(),P.end(),p2);
    memcpy(p_predict2,p_predict,4*sizeof(FLOAT_M));
    for (int32_t j=0; j<4; j++) {
